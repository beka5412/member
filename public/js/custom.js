...
function res_filter(x) {
    if (x.matches) { // If media query matches
        // document.body.style.backgroundColor = "yellow";
        $('#filter-div').removeClass('col-9');
        $('#filter-div').addClass('col-12');
    } else {
        // document.body.style.backgroundColor = "pink";
        $('#filter-div').removeClass('col-12');
        $('#filter-div').addClass('col-9');
    }
}

var x = window.matchMedia("(max-width: 991px)")
res_filter(x) // Call listener function at run time
x.addListener(res_filter) // Attach listener function on state changes
...
$(document).on('click','.dataTable-pagination-list',function(){
    commonLoader();
});
...
$(function(){
    $(document).on("click",".show_confirm_link",function(){
        const swalWithBootstrapButtons = Swal.mixin({
            customClass: {
                confirmButton: 'btn btn-success',
                cancelButton: 'btn btn-danger'
